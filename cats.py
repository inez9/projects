...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def func(paragraphs): 
        clean = remove_punctuation(paragraphs).lower().split()
        for i in subject: 
            for x in clean: 
                if x == i:
                    return True
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]: 
            sameness += 1
    return float((sameness/len(typed_words))*100)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    closest_match = min(difference, key=difference.get)
...
def time_per_word(words, timestamps_per_player):
    ...
    list of words and the amount of time each player took to type each 4word.
    ...
    for i in timestamps_per_player: 
        ...
        for x in range(1, len(i)): 
           mini_lst.append(i[x] - i[x-1])  
        new_lst.append(mini_lst)
    return match(words, new_lst)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    lst = [] * len(get_all_times(match))
    for i in player_indices: 
        player = []
        lst.append(player)
    if not word_indices: 
        return lst
    min_speed = time(match, 0 , 0)
    winner = 0 
    for word in word_indices: 
        min_speed = time(match, 0 , word)
        for i in player_indices: 
            if min_speed > time(match, i, word): 
                min_speed = time(match, i, word)
                winner = i 
        fastest_word = get_word(match, word)
        lst[winner].append(fastest_word)
        winner = 0 
    return lst
    # END PROBLEM 10
